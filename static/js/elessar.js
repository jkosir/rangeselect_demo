...
// thanks to http://my.opera.com/emoller/blog/2011/12/20/requestanimationframe-for-smart-er-animating
            ...
            var Range = Element.extend(vertical).extend({
                initialize: function initialize(options) {
                    ...
                    this.build = this.$el.append('<div class="elessar-block-build">Build & Peak</div>');
                    if (this.perant.ranges.length > 0 && this.options.label != '+') {
                        var remove_btn = $('<div class="elessar-range-remove"><i class="fa fa-times"></i></div>');
                        this.$el.append(remove_btn);
                        remove_btn.click(function (e) {
                            self.perant.removeRange(self);
                        });
                    }
                ...
                val: function (range, valOpts) {
                    ...
                    if (next && next.val()[0] < range[1]) {
                        ...
                        if (next.val()[1] - range[1] >= 9 / 52) {
                            this.perant.repositionRange(this, range);
                            next.val([range[1], next.val()[1]]);
                        } else {
                            range[1] = next.val()[0];
                    ...
                    if (this.options.label === '+') {
                        ...
                        if (prev && prev.val()[1] > range[0]) {
                            if (range[0] - prev.val()[0] >= 9 / 52) {
                                this.perant.repositionRange(this, range);
                                prev.val([prev.val()[0], range[0]]);
                            } else {
                                range[0] = prev.val()[1];
                ...
                resizeEnd: function (origEv) {
                    var self = this,
                        ...
                        maxSize = this.options.maxSize * perantSize;
            ...
            var RangeBar = Element.extend(vertical).extend({
                ...
                squeezeRanges: function () {
                    var self = this;
                    $(self.ranges).each(function (i, range) {
                        var size = range.val()[1] - range.val()[0];
                        if (i == 0) {
                            range.val([0, size]);
                        }
                        var next = self.nextRange(range.$el);
                        if (next) {
                            var next_size = next.val()[1] - next.val()[0];
                            next.val([range.val()[1], range.val()[1] + next_size]);
                        }
                ...
                addRange: function (range, data) {
                    var $range = Range({
                        ...
                        maxSize: this.options.maxSize ? this.abnormaliseRaw(this.options.maxSize - this.options.min) : null,
                    ...
                    $('.elessar-block-build').width(8 * ($('.elessar-rangebar').width() / 52));
                    return $range;
                },
                ...
                mousemove: function (ev) {
                    ...
                    if ((direct || phantom) && this.ranges.length < this.options.maxRanges && !$('body').is('.elessar-dragging, .elessar-resizing') && !this.readonly()) {
                        ...
                        $('.elessar-phantom .elessar-block-build').width(8 * ($('.elessar-rangebar').width() / 52)).text('Add peak');
